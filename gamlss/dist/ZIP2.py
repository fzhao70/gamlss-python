"""Zero Inflated Poisson type 2 distribution (ZIP2). Port of gamlss.dist
R/ZIP2.R.
...
def _zip2_dldm(y, mu, sigma):
    with np.errstate(over="ignore"):
        mus = mu / (1 - sigma)
        dldm0 = -(((1 - sigma) + sigma * np.exp(mus)) ** (-1))
        dldm = np.where(y == 0, dldm0, (y - mus) / mu)
    return dldm


def _zip2_dldd(y, mu, sigma):
    mus = mu / (1 - sigma)
    dldd0 = ((1 - (1 + mus) * np.exp(-mus))
             * ((sigma + (1 - sigma) * np.exp(-mus)) ** (-1)))
    dldd = np.where(y == 0, dldd0, ((y - 1) / (1 - sigma)) - ((mus**2) / mu))
    return dldd


def ZIP2(mu_link="log", sigma_link="logit"):
    mstats = checklink("mu.link", "ZIP2", mu_link,
                       ("1/mu^2", "log", "identity"))
    dstats = checklink("sigma.link", "ZIP2", sigma_link,
                       ("logit", "probit", "cloglog", "log"))

    def d2ldm2(y, mu, sigma):
        dldm = _zip2_dldm(y, mu, sigma)
        d2ldm2 = -dldm * dldm
        d2ldm2 = np.where(d2ldm2 < -1e-15, d2ldm2, -1e-15)
        return d2ldm2

    def d2ldd2(y, mu, sigma):
        dldd = _zip2_dldd(y, mu, sigma)
        d2ldd2 = -dldd * dldd
    ...
    return GamlssFamily(
        family=("ZIP2", "Zero Inflated Poisson 2"),
        ...
        G_dev_incr=lambda y, mu, sigma: -2 * dZIP2(y, mu, sigma, log=True),
        rqres={"pfun": "pZIP2", "type": "Discrete", "ymin": 0},
        initial={
        ...
        variance=lambda mu, sigma: mu * (1 + (mu * sigma) / (1 - sigma)),
    )


def dZIP2(x, mu=5, sigma=0.1, log=False):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        logfy = np.where(
            x == 0,
            np.log(sigma + (1 - sigma) * np.exp(-mus)),
            ((1 - x) * np.log(1 - sigma) - mus + x * np.log(mu)
             - _sp.gammaln(x + 1)),
        )
...
def pZIP2(q, mu=5, sigma=0.1, lower_tail=True, log_p=False):
    if np.any(np.asarray(mu) <= 0):
        raise ValueError("mu must be greater than 0")
    if np.any(np.asarray(sigma) <= 0) or np.any(np.asarray(sigma) >= 1):
        raise ValueError("sigma must be between 0 and 1")
    ...
    mus = mu / (1 - sigma)
    cdf = _st.poisson.cdf(np.floor(q), mus)
...
def qZIP2(p, mu=5, sigma=0.1, lower_tail=True, log_p=False):
    if np.any(np.asarray(mu) <= 0):
        raise ValueError("mu must be greater than 0")
    ...
    with np.errstate(invalid="ignore"):
        q = np.where(pnew > 0, _st.poisson.ppf(pnew, mus), 0)
    return q


def rZIP2(n, mu=5, sigma=0.1, rng=None):
    if np.any(np.asarray(mu) <= 0):
